...
function isDown(code){ return keys.has(code) || !!vkeys[code]; }
function setVKey(code, down){ vkeys[code] = down; }
...
const touchBtns = touchWrap ? touchWrap.querySelectorAll(".tcBtn") : [];

function enableTouchUI(){
  if(!touchWrap) return;
  const isTouch = matchMedia("(hover: none) and (pointer: coarse)").matches || matchMedia("(max-width: 900px)").matches;
  touchWrap.style.display = isTouch ? "block" : "none";
  touchWrap.setAttribute("aria-hidden", isTouch ? "false" : "true");
}
enableTouchUI();
window.addEventListener("resize", enableTouchUI);
...
function bindTouch(btn){
  ...
  const down = (e)=>{
    ...
    // iOS needs gesture for audio context
...
touchBtns.forEach(bindTouch);

document.addEventListener("touchmove", (e)=>{
  if(matchMedia("(hover: none) and (pointer: coarse)").matches) e.preventDefault();
}, {passive:false});
...
function update(dt){
  ...
  const maxV = 340;
  let ax = 0;
  if(isDown("ArrowLeft"))  { ax -= accel; hero.face=-1; }
  if(isDown("ArrowRight")) { ax += accel; hero.face= 1; }
  hero.vx += ax*dt;
  if(ax===0) hero.vx *= Math.pow(0.001, dt);
  hero.vx = clamp(hero.vx, -maxV, maxV);

  if(isDown("Space") && !hero.jumpBuffered){
  ...
  if(!isDown("Space")) hero.jumpBuffered = false;

  if(isDown("ArrowUp") && hero.fuel > 0){
    hero.vy -= 980*dt;
...
function drawHero(){
  ...
  if(isDown("ArrowUp") && hero.fuel>0.02){
...
function drawIntroCutscene(){
  ...
  ctx.fillText("DEUTSCH ECHO — STADTRETTUNG", W/2, py+20);
  ...
  const lines = [
    "Monster haben die Stadt übernommen und die Prinzessin eingesperrt.",
    "Nur ein Held kann sie retten — aber ohne dich schafft er es nicht.",
    "Errate 5 Wörter richtig, um einen Schlüssel zu bekommen.",
    "Die Monster lassen dich nicht einfach gewinnen.",
    "Mit X kannst du schießen, damit du nicht getroffen wirst.",
    "Alles hängt von dir ab.",
    "",
    "Drücke START, um das Spiel zu beginnen."
  ];
  ...
  ctx.fillText("Deutsch Echo — نجات شهر", W/2, py+ph-18);
